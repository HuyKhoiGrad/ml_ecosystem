...
def get_source_data(
    curr_time: str, cols: list, source_db: YugaByteDBController
) -> pd.DataFrame:
...
def exec_ingest_source_data(latest_checkpoint=None):
    ...
    try:
        ...
        if latest_checkpoint == None:
            latest_checkpoint = redis.getDataByKey(
                name=REDIS_INFERENCE_NAME, value=REDIS_INFERENCE_KEY
            )
        current_df = get_source_data(
            curr_time=latest_checkpoint, cols=SOURCE_DATA_COLUMNS, source_db=source_db
        )
