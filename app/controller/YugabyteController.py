...
class YugaByteDBController:
    def __init__(self):
        super().__init__()
        self.params = YugabyteConfig().get_config()
    ...
    def insert_data(
        self, table_name: str, update_data: pd.DataFrame, constraint_key: str
    ):
        ...
        for data in list_records:
            try:
                ...
                v_sql = (
                    v_sql
                    + " on conflict on constraint %s do update set " % constraint_key
                )
                ...
                print(str(ex))
                logger.info(
                    "[SQL: {} - With Data: {} - Error: {}]".format(
                        v_sql, list(data.values()), str(ex)
                    )
                )
